...
func Run(cfg *config.ApplicationConfig) {
	...
	queueUseCase := usecase.NewQueueUseCase(queueRepo, queueSlotsRepo, userRepo)
	...
	queueHandler := http2.NewQueueHandler(queueUseCase, groupUseCase)
	...
	adminHandler := http2.NewAdminHandler(nil, groupUseCase) // TODO: create AdminUseCase
