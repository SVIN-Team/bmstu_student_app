...
type QueueSlotsHandler struct {
	...
	userUseCase       UserUseCase // Добавлен userUseCase
}

func NewQueueSlotsHandler(queueSlotsUseCase QueueSlotsUseCase, queueUseCase QueueUseCase, userUseCase UserUseCase) *QueueSlotsHandler {
	return &QueueSlotsHandler{
		queueSlotsUseCase: queueSlotsUseCase,
		queueUseCase:      queueUseCase,
		userUseCase:       userUseCase, // Инициализация
	}
}
...
func (h *QueueSlotsHandler) GetQueueSlots(ctx *gin.Context) {
	...
	for i, slot := range slots {
		response[i] = h.slotToDTO(ctx, slot, false)
	}
...
func (h *QueueSlotsHandler) GetQueueSlot(ctx *gin.Context) {
	...
	SuccessResponse(ctx, http.StatusOK, h.slotToDTO(ctx, slot, true))
}
...
func (h *QueueSlotsHandler) SignUpForQueue(ctx *gin.Context) {
	...
	SuccessResponse(ctx, http.StatusCreated, h.slotToDTO(ctx, *newSlot, true))
}
...
// @Failure 409 {object} dto.ErrorResponse
// @Failure 500 {object} dto.ErrorResponse
// @Router /queues/{queue_id}/slots/me [delete]
...
func (h *QueueSlotsHandler) TransferFailedSlots(ctx *gin.Context) {
	...
	for _, slot := range slots {
		response = append(response, h.slotToDTO(ctx, slot, false))
	}
...
func (h *QueueSlotsHandler) slotToDTO(ctx context.Context, slot models.QueueSlot, detailed bool) dto.SlotResponse {
	var firstName, lastName string
	if h.userUseCase != nil {
		user, err := h.userUseCase.GetUserByID(ctx, slot.StudentID)
		if err == nil {
			firstName = user.FirstName
			lastName = user.LastName
		} else {
			logger.Errorf(ctx, "Failed to get user for slotDTO: %v", err)
		}
	}
